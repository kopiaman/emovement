...
include("include/User_Level_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "User_Level_inline_edit.htm" : "User_Level_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["UserName"] = urldecode(postvalue("editid1"));
$savedKeys["UserName"] = urldecode(postvalue("editid1"));
$skeys.= rawurlencode(postvalue("editid1"))."&";
$keys["GroupID"] = urldecode(postvalue("editid2"));
$savedKeys["GroupID"] = urldecode(postvalue("editid2"));
...
//	processing GroupID - begin
	$condition = 1;

	if($condition)
	{
		$control_GroupID = $pageObject->getControl("GroupID", $id);
		$control_GroupID->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			//	update key value
		if($control_GroupID->getWebValue()!==false)
			$keys["GroupID"] = $control_GroupID->getWebValue();
	}
//	processing GroupID - end
	...
	//	do event
		...
		{		
			...
			//	processing GroupID - begin
				...
				if($condition)
				{
					$control_GroupID->afterSuccessfulSave();
				}
	//	processing GroupID - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["UserName"])."&";
		$keyGetQ.="editid2=".rawurldecode($keys["GroupID"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: User_Level_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: User_Level_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("GroupID"))
		$xt->assign("GroupID_fieldblock",true);
	else
		$xt->assign("GroupID_tabfieldblock",true);
	$xt->assign("GroupID_label",true);
	if(isEnableSection508())
		$xt->assign_section("GroupID_label","<label for=\"".GetInputElementId("GroupID", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("UserName", $data)));
	$xt->assign("show_key2", htmlspecialchars($pageObject->showDBValue("GroupID", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["UserName"]));
	$keylink.= "&key2=".htmlspecialchars(rawurlencode(@$data["GroupID"]));
...
//	GroupID - 
	$value = $pageObject->showDBValue("GroupID", $data, $keylink);
	$showValues["GroupID"] = $value;
	$showFields[] = "GroupID";
		$showRawValues["GroupID"] = substr($data["GroupID"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "User_Level";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='User_Level_view.php?".$viewlink."'\"");
