...
$layout = new TLayout("register11","ExtravaganzaOrange1","MobileOrange1");
...
$layout = new TLayout("register_success1","ExtravaganzaOrange1","MobileOrange1");
$layout->blocks["top"] = array();
$layout->containers["fields"] = array();
...
//	processing icno - start
	...
	$control_icno->readWebValue($values, $blobfields, "", null, $filename_values);
//	processibng icno - end

//	processing pass - start
	$control_pass = $pageObject->getControl("pass", $id);
	$control_pass->readWebValue($values, $blobfields, "", null, $filename_values);
//	processibng pass - end

//	processing fullname - start
	$control_fullname = $pageObject->getControl("fullname", $id);
	$control_fullname->readWebValue($values, $blobfields, "", null, $filename_values);
//	processibng fullname - end

//	processing pos - start
	$control_pos = $pageObject->getControl("pos", $id);
	$control_pos->readWebValue($values, $blobfields, "", null, $filename_values);
//	processibng pos - end

//	processing umail - start
	$control_umail = $pageObject->getControl("umail", $id);
	$control_umail->readWebValue($values, $blobfields, "", null, $filename_values);
//	processibng umail - end

//	processing department - start
	$control_department = $pageObject->getControl("department", $id);
	$control_department->readWebValue($values, $blobfields, "", null, $filename_values);
//	processibng department - end

//	processing subdepartment - start
	$control_subdepartment = $pageObject->getControl("subdepartment", $id);
	$control_subdepartment->readWebValue($values, $blobfields, "", null, $filename_values);
//	processibng subdepartment - end

//	processing sstatus - start
	$control_sstatus = $pageObject->getControl("sstatus", $id);
	$control_sstatus->readWebValue($values, $blobfields, "", null, $filename_values);
//	processibng sstatus - end

	$strUsername = $values["icno"];
	$strPassword = $values["pass"];
	$strEmail = $values["umail"];

	if($cipherer->isFieldEncrypted("icno"))
		$sUsername = $cipherer->MakeDBValue("icno",$strUsername,"","",true);	
	else 
		$sUsername = add_db_quotes("icno",$strUsername);
	
	if($cipherer->isFieldEncrypted("umail"))
		$sEmail = $cipherer->MakeDBValue("umail",$strEmail,"","",true);	
	else 
		$sEmail = add_db_quotes("umail",$strEmail);
...
//	encrypt password
		...
		$message = "";
		$retval = DoInsertRecord("user",$values,$blobfields,$id,$pageObject, $cipherer);
		$strMessage = $message;
...
//	send email to user
		...
		$strLabel = "Username";
		$message.= $strLabel.": ".$values["icno"]."\r\n";
		$strLabel = "Fullname";
		$message.= $strLabel.": ".$values["fullname"]."\r\n";
		$strLabel = "Position";
		$message.= $strLabel.": ".$values["pos"]."\r\n";
		$strLabel = "Email";
		$message.= $strLabel.": ".$values["umail"]."\r\n";
		$strLabel = "Function";
		$message.= $strLabel.": ".$values["department"]."\r\n";
		$strLabel = "Sector";
		$message.= $strLabel.": ".$values["subdepartment"]."\r\n";
		$strLabel = "Status";
		$message.= $strLabel.": ".$values["sstatus"]."\r\n";
...
//	send letter to admin
		...
		$message.= $strLabel.": ".$values["sstatus"]."\r\n";
		$sentMailResults = runner_mail(array('to' => "kopiaman@yahoo.com", 'subject' => "Notification on registering", 'body' => $message));
...
//	show readonly fields
	$pageObject->readOnlyFields["sstatus"] = htmlspecialchars($pageObject->showDBValue("sstatus", $values));
