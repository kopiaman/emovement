...
if($inlineadd == ADD_INLINE)
	$templatefile = "set_setting_category_inline_add.htm";
else
	$templatefile = "set_setting_category_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: set_setting_category_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing category_name - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_category_name = $pageObject->getControl("category_name", $id);
		$control_category_name->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing category_name - end
...
//	processing category_name - start
			...
			if($inlineAddOption)
			{
				$control_category_name->afterSuccessfulSave();
			}
//	processing category_name - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'set_setting_category_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'set_setting_category_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: set_setting_category_".$pageObject->getPageType().".php");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["setting"]["masterkey1"] = $data["id"];	
...
//	category_name
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("category_name", $data, $keylink);
		$showValues["category_name"] = $value;
		$showFields[] = "category_name";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["category_name"] = substr($data["category_name"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "set_setting_category";
		$options['firstTime'] = 1;
