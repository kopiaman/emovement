...
include("include/training_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "training_inline_edit.htm" : "training_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["tid"] = urldecode(postvalue("editid1"));
$savedKeys["tid"] = urldecode(postvalue("editid1"));
...
//	processing files - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_files = $pageObject->getControl("files", $id);
		$control_files->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
	...
	//	do event
		...
		{		
			...
			//	processing files - begin
				...
				if($condition)
				{
					$control_files->afterSuccessfulSave();
				}
	//	processing files - end
			//	processing notes - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["tid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: training_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: training_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("tid", $data)));
...
//	tid - 
	$value = $pageObject->showDBValue("tid", $data, $keylink);
	$showValues["tid"] = $value;
	$showFields[] = "tid";
		$showRawValues["tid"] = substr($data["tid"],0,100);

//	files - Custom
	$value = $pageObject->showDBValue("files", $data, $keylink);
	$showValues["files"] = $value;
	$showFields[] = "files";
		$showRawValues["files"] = substr($data["files"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='training_view.php?".$viewlink."'\"");
