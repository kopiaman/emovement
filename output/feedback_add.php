...
$layout->blocks["top"][] = "details";$page_layouts["feedback_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "feedback_inline_add.htm";
else
	$templatefile = "feedback_add.htm";
...
//	processing dates - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_dates = $pageObject->getControl("dates", $id);
		$control_dates->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing dates - end
//	processing categories - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_categories = $pageObject->getControl("categories", $id);
		$control_categories->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing categories - end
//	processing feedback - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_feedback = $pageObject->getControl("feedback", $id);
		$control_feedback->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'feedback_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'feedback_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: feedback_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["fid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["fid"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["dates"]=@$avalues["dates"];
	$defvalues["categories"]=@$avalues["categories"];
	$defvalues["feedback"]=@$avalues["feedback"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("dates_label","<label for=\"".GetInputElementId("dates", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("categories_label","<label for=\"".GetInputElementId("categories", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("feedback_label","<label for=\"".GetInputElementId("feedback", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["fid"]));
...
//	dates
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("dates", $data, $keylink);
		$showValues["dates"] = $value;
		$showFields[] = "dates";
	}	
//	categories
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("categories", $data, $keylink);
		$showValues["categories"] = $value;
...
//	feedback
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("feedback", $data, $keylink);
		$showValues["feedback"] = $value;
		$showFields[] = "feedback";
	}	
//	reply
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("reply", $data, $keylink);
		$showValues["reply"] = $value;
		$showFields[] = "reply";
	}	
//	fstatus
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("fstatus", $data, $keylink);
		$showValues["fstatus"] = $value;
		$showFields[] = "fstatus";
	}	
//	attachment
	$display = false;
	...
	{	
		...
		$showRawValues["dates"] = substr($data["dates"],0,100);
		$showRawValues["categories"] = substr($data["categories"],0,100);
		$showRawValues["feedback"] = substr($data["feedback"],0,100);
		$showRawValues["reply"] = substr($data["reply"],0,100);
		$showRawValues["fstatus"] = substr($data["fstatus"],0,100);
		$showRawValues["fdate"] = substr($data["fdate"],0,100);
