...
if($inlineadd == ADD_INLINE)
	$templatefile = "training_inline_add.htm";
else
	$templatefile = "training_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: training_add.php');
...
//	processing files - start
	...
	if($inlineAddOption)
	{
		...
		$control_files->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'training_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'training_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: training_".$pageObject->getPageType().".php");
