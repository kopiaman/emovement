...
{	
	...
	*/
	...
	{	
		...
		$userheaderlink_attrs = "href=\"".$this->tName."_list.php?orderby=Aicno\"";
		
		//for display name 
		$displayuserheaderlink_attrs = "href=\"".$this->tName."_list.php?orderby="."Afullname\"";
		
		if ($this->sortByGroup === false && $this->sortField != ""){
			if ($this->sortField == "icno"){
	...
	function save() 
	{
		if(postvalue("a")=="save")
		{
			...
			foreach(postvalue("username") as $user)
			{
				...
				$sql="delete from `emov_ugmembers` where UserName=".db_prepare_string($user);
				...
				if(count($_POST["cb_".$useridx]))
				{
					...
					foreach(@$_POST["cb_".$useridx] as $g)
					{
						if($g<0)
						{
							if($user==$_SESSION["UserID"] && $g==-1)
								continue;
							$sql="insert into `emov_ugmembers` (UserName,GroupID) values (".db_prepare_string(htmlspecialchars($user)).",".$g.")";
							db_exec($sql,$this->conn);
						}
						else
						{
					...
					if($glist!="")
					{
						$sql="insert into `emov_ugmembers` (UserName,GroupID) select ".db_prepare_string(htmlspecialchars($user)).",GroupID from `emov_uggroups` where GroupID in (".$glist.")";
						db_exec($sql,$this->conn);
					}
				}
			}
	...
	function fillGridData() 
	{
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			foreach($this->groups as $idx => $g)
			{
				...
				if($_SESSION["UserID"] != $data["icno"] || $g[0] != -1)
				{
			...
			//$row["begin"] = "<input type=hidden name=\"username[]\" value=\"".htmlspecialchars($data["icno"])."\">";
			
			$row["username"] = htmlspecialchars($data["icno"]);
			$row["displayusername"] = htmlspecialchars($data["fullname"]); //htmlspecialchars($data["icno"]);
			$row["user"] = $data["icno"];
			$usercheckbox_attrs.=" name=\"user_".count($this->users)
				."\" id=\"".count($this->users)."\"";
			
			$this->users[] = htmlspecialchars($data["icno"]);
	...
	{
		//	select members list		
		$trs = db_query("select `UserName`,`GroupID` from `emov_ugmembers` order by `UserName`,`GroupID`",$this->conn);
	...
	function fillGroups() 
	{
		...
		$trs = db_query("select `GroupID`,`Label` from `emov_uggroups` order by `Label`",$this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
	...
	function setSessionVariables() 
	{
		...
		if (substr($_SESSION[$this->sessionPrefix."_orderby"],1) == "icno" || substr($_SESSION[$this->sessionPrefix."_orderby"],1) =="fullname"){
