...
include("include/feedback_variables.php");
...
$layout = new TLayout("edit2","ExtravaganzaOrange1","MobileOrange1");
...
$layout->blocks["top"][] = "details";$page_layouts["feedback_edit"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["feedback_edit_Reply1"] = $layout;
...
$templatefile = "feedback_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["fid"] = urldecode(postvalue("editid1"));
$savedKeys["fid"] = urldecode(postvalue("editid1"));
...
//	processing dates - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_dates = $pageObject->getControl("dates", $id);
		$control_dates->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing dates - end
//	processing categories - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_categories = $pageObject->getControl("categories", $id);
		$control_categories->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing categories - end
//	processing feedback - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_feedback = $pageObject->getControl("feedback", $id);
		$control_feedback->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing feedback - end
//	processing reply - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_reply = $pageObject->getControl("reply", $id);
		$control_reply->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing reply - end
//	processing fstatus - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_fstatus = $pageObject->getControl("fstatus", $id);
		$control_fstatus->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing fstatus - end
//	processing fdate - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_fdate = $pageObject->getControl("fdate", $id);
		$control_fdate->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing fdate - end
//	processing attachment - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_attachment = $pageObject->getControl("attachment", $id);
		$control_attachment->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing attachment - end
	...
	//	processing title - end
			//	processing dates - begin
				...
				if($condition)
				{
					$control_dates->afterSuccessfulSave();
				}
	//	processing dates - end
			//	processing categories - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_categories->afterSuccessfulSave();
				}
	//	processing categories - end
			//	processing feedback - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_feedback->afterSuccessfulSave();
				}
	//	processing feedback - end
			//	processing reply - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_reply->afterSuccessfulSave();
				}
	//	processing reply - end
			//	processing fstatus - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_fstatus->afterSuccessfulSave();
				}
	//	processing fstatus - end
			//	processing fdate - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_fdate->afterSuccessfulSave();
				}
	//	processing fdate - end
			//	processing attachment - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["fid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: feedback_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: feedback_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["dates"] = $evalues["dates"];
	$data["categories"] = $evalues["categories"];
	$data["feedback"] = $evalues["feedback"];
	$data["reply"] = $evalues["reply"];
	$data["fstatus"] = $evalues["fstatus"];
	$data["fdate"] = $evalues["fdate"];
	$data["attachment"] = $evalues["attachment"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("title_label","<label for=\"".GetInputElementId("title", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("dates"))
		$xt->assign("dates_fieldblock",true);
	else
		$xt->assign("dates_tabfieldblock",true);
	$xt->assign("dates_label",true);
	if(isEnableSection508())
		$xt->assign_section("dates_label","<label for=\"".GetInputElementId("dates", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("categories"))
		$xt->assign("categories_fieldblock",true);
	else
		$xt->assign("categories_tabfieldblock",true);
	$xt->assign("categories_label",true);
	if(isEnableSection508())
		$xt->assign_section("categories_label","<label for=\"".GetInputElementId("categories", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("feedback"))
		$xt->assign("feedback_fieldblock",true);
	else
		$xt->assign("feedback_tabfieldblock",true);
	$xt->assign("feedback_label",true);
	if(isEnableSection508())
		$xt->assign_section("feedback_label","<label for=\"".GetInputElementId("feedback", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("reply"))
		$xt->assign("reply_fieldblock",true);
	else
		$xt->assign("reply_tabfieldblock",true);
	$xt->assign("reply_label",true);
	if(isEnableSection508())
		$xt->assign_section("reply_label","<label for=\"".GetInputElementId("reply", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("fstatus"))
		$xt->assign("fstatus_fieldblock",true);
	else
		$xt->assign("fstatus_tabfieldblock",true);
	$xt->assign("fstatus_label",true);
	if(isEnableSection508())
		$xt->assign_section("fstatus_label","<label for=\"".GetInputElementId("fstatus", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("fdate"))
		$xt->assign("fdate_fieldblock",true);
	else
		$xt->assign("fdate_tabfieldblock",true);
	$xt->assign("fdate_label",true);
	if(isEnableSection508())
		$xt->assign_section("fdate_label","<label for=\"".GetInputElementId("fdate", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("attachment_label","<label for=\"".GetInputElementId("attachment", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("fid", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='feedback_view.php?".$viewlink."'\"");
