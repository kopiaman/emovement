...
$templatefile = ($inlineedit == EDIT_INLINE) ? "User_Access_Level_inline_edit.htm" : "User_Access_Level_edit.htm";
...
//	processing subdepartment - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_subdepartment = $pageObject->getControl("subdepartment", $id);
		$control_subdepartment->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing subdepartment - end
//	processing umail - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_umail = $pageObject->getControl("umail", $id);
		$control_umail->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing umail - end
...
//	processing icno - end
//	processing pos - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_pos = $pageObject->getControl("pos", $id);
		$control_pos->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing sstatus - end
	...
	//	processing department - end
			//	processing subdepartment - begin
				...
				if($condition)
				{
					$control_subdepartment->afterSuccessfulSave();
				}
	//	processing subdepartment - end
			//	processing umail - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_umail->afterSuccessfulSave();
				}
	//	processing umail - end
			//	processing icno - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
	...
	//	processing icno - end
			//	processing pos - begin
				...
				if($condition)
				{
					$control_pos->afterSuccessfulSave();
				}
	//	processing pos - end
			//	processing sstatus - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: User_Access_Level_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: User_Access_Level_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["subdepartment"] = $evalues["subdepartment"];
	$data["umail"] = $evalues["umail"];
	$data["icno"] = $evalues["icno"];
	$data["pos"] = $evalues["pos"];
...
//	detail tables
	$showDetailKeys["User_Level"]["masterkey1"] = $data["icno"];		
...
//	umail - 
	$value = $pageObject->showDBValue("umail", $data, $keylink);
	$showValues["umail"] = $value;
	$showFields[] = "umail";
		$showRawValues["umail"] = substr($data["umail"],0,100);
...
//	pos - 
	$value = $pageObject->showDBValue("pos", $data, $keylink);
	$showValues["pos"] = $value;
	$showFields[] = "pos";
...
//	totalrecord - 
	$value = $pageObject->showDBValue("totalrecord", $data, $keylink);
	$showValues["totalrecord"] = $value;
	$showFields[] = "totalrecord";
		$showRawValues["totalrecord"] = substr($data["totalrecord"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "User Access Level";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "User Access Level";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='User_Access_Level_view.php?".$viewlink."'\"");
