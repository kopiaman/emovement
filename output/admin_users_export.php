...
$layout = new TLayout("export","ExtravaganzaOrange1","MobileOrange1");
...
function ExportToXML($cipherer)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["fullname"] = $pageObject->showDBValue("fullname", $row);
			$values["department"] = $pageObject->showDBValue("department", $row);
			$values["subdepartment"] = $pageObject->showDBValue("subdepartment", $row);
			$values["umail"] = $pageObject->showDBValue("umail", $row);
			$values["pos"] = $pageObject->showDBValue("pos", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["fullname"] = $pageObject->getViewControl("fullname")->showDBValue($row, "");
			$values["department"] = $pageObject->getViewControl("department")->showDBValue($row, "");
			$values["subdepartment"] = $pageObject->getViewControl("subdepartment")->showDBValue($row, "");
			$values["umail"] = $pageObject->getViewControl("umail")->showDBValue($row, "");
			$values["pos"] = $pageObject->getViewControl("pos")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["fullname"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["department"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["subdepartment"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["umail"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["pos"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Fullname").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Function").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Sector").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["fullname"]);
					else
			...
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["subdepartment"]);
				else
					echo $values["subdepartment"];//echo htmlspecialchars($values["subdepartment"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["umail"]);
					else
						echo $values["umail"];
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["pos"]);
				else
					echo $values["pos"];//echo htmlspecialchars($values["pos"]); commented for bug #6823
