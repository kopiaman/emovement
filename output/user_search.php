...
$layout->blocks["top"][] = "search";$page_layouts["user_search"] = $layout;
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("pass"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("pass")] = GetTableURL($pageObject->pSet->getLookupTable("pass"));
	
	$pageObject->fillFieldToolTips("pass");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("pass");
	...
	if(isEnableSection508())
		$xt->assign_section("pass_label","<label for=\"".GetInputElementId("pass", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("pass_label", true);
	
	$xt->assign("pass_fieldblock", true);
	$xt->assignbyref("pass_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("pass_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("pass_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_pass", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("pass");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"pass", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"pass", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
