...
if($inlineadd == ADD_INLINE)
	$templatefile = "User_Level_inline_add.htm";
else
	$templatefile = "User_Level_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: User_Level_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing GroupID - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_GroupID = $pageObject->getControl("GroupID", $id);
		$control_GroupID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing GroupID - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="User Access Level")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["UserName"]==""){
			$avalues["UserName"] = prepare_for_db("UserName",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'User_Level_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'User_Level_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: User_Level_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["UserName"]=postvalue("copyid1");
		$copykeys["GroupID"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["UserName"]=postvalue("editid1");
		$copykeys["GroupID"]=postvalue("editid2");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="User Access Level")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["UserName"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["GroupID"]=@$avalues["GroupID"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("GroupID_label","<label for=\"".GetInputElementId("GroupID", $id, PAGE_ADD)."\">","</label>");
