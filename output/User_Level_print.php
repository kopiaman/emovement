...
$layout->blocks["top"][] = "pdf";$page_layouts["User_Level_print"] = $layout;
...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["UserName"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$keys["GroupID"]=refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<2)
				continue;
			$keys=array();
			$keys["UserName"]=urldecode($arr[0]);
			$keys["GroupID"]=urldecode($arr[1]);
			$selected_recs[]=$keys;
		}
	}
...
//	GroupID - 
			$record["GroupID_value"] = $pageObject->showDBValue("GroupID", $data, $keylink);
			$record["GroupID_class"] = $pageObject->fieldClass("GroupID");
			if($col<$colsonpage)
...
$xt->assign("GroupID_fieldheadercolumn",true);
$xt->assign("GroupID_fieldheader",true);
$xt->assign("GroupID_fieldcolumn",true);
$xt->assign("GroupID_fieldfootercolumn",true);
