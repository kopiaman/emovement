...
function runner_mail($params)
{
	...
	if(!$from)
	{
		$from = "no-reply@emovement.bptv.com";
	}
...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="timetable" && $field=="sstatus" && $ptype=="view")
	{
		$status=$data['sstatus'];
if($status=="Pending"){
$value ="<font color ='magenta'><strong>$status</strong></font>";
...
$value ="<font color ='green'><strong>$status</strong></font>";};
		return $value;
				...
				if($table=="timetable" && $field=="sstatus" && $ptype=="list")
	{
		$status=$data['sstatus'];
if($status=="Pending"){
$value ="<font color ='magenta'><strong>$status</strong></font>";
...
$value ="<font color ='green'><strong>$status</strong></font>";};
		return $value;
				...
				if($table=="my_timetable" && $field=="sstatus")
	{
		$status=$data['sstatus'];
if($status=="Pending"){
$value ="<font color ='magenta'><strong>$status</strong></font>";
...
$value ="<font color ='green'><strong>$status</strong></font>";};
		return $value;
				...
				if($table=="User Access Level" && $field=="sstatus" && $ptype=="view")
	{
		$status=$data['sstatus'];
if($status=="Pending"){
...
}else{ $value ="<font color ='green'>$status</font>";};
		return $value;
	}	
				if($table=="User Access Level" && $field=="sstatus" && $ptype=="list")
	{
		$status=$data['sstatus'];
if($status=="Pending"){
$value ="<font color ='red'><strong>$status</strong></font>";
...
$value ="<font color ='Blue'>$status</font>";
		...
		return $value;
				...
				if($table=="training" && $field=="files")
	{
		
$value1=$data['files'];

if ($value1){
$filesArray = json_decode($value1);
$filenames = $filesArray[0]->name;
$orinames = $filesArray[0]->usrName;
$thumbnames = $filesArray[0]->thumbnail;
}
//$value = $filenames;
$value="<span><a href='$filenames' target='_blank'> $orinames </a></span>";
/*
$whitelist = array('127.0.0.1', "::1");
if(!in_array($_SERVER['REMOTE_ADDR'], $whitelist)){
    $value="<span><a href='http://ihijrah.com/emovement/$filenames' target='_blank'> $filenames </a></span>";
}else{
$value="<span><a href='$filenames' target='_blank'> $filenames </a></span>";
};
*/
;
		return $value;
...
function fileCustomExpression($file, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="timetable" && $field=="attachment")
	{
		;
		return $value;
	}	
				if($table=="my_timetable" && $field=="attachment")
	{
		;
		return $value;
	}	
				if($table=="feedback" && $field=="attachment")
	{
		;
		return $value;
	}	
...
function GetLWWhere($field, $ptype, $table = "")
{
	...
	if(!$table) 
		$table = $strTableName;
				if($table=="timetable" && $field=="category" && $ptype=="edit")
	{
		return "Class=30";
	}
				if($table=="timetable" && $field=="category" && $ptype=="register")
	{
		return "Class=50";
	}
				if($table=="timetable" && $field=="category" && $ptype=="add")
	{
		return "Class=30";
	}
				if($table=="timetable" && $field=="category" && $ptype=="search")
	{
		return "Class=30";
	}
				if($table=="timetable" && $field=="level")
	{
		return "Class=40";
	}
				if($table=="timetable" && $field=="pos")
	{
		return "Class=50 AND Value!=40";
	}
				if($table=="admin_users" && $field=="pos")
	{
		return "Class=50";
	}
				if($table=="user" && $field=="pos")
	{
		return "Class=50";
	}
				if($table=="user_profile" && $field=="pos")
	{
		return "Class=50";
	}
				if($table=="alltest" && $field=="department")
	{
		return "Class=20";
	}
				if($table=="my_timetable" && $field=="category" && $ptype=="edit")
	{
		return "Class=30";
	}
				if($table=="my_timetable" && $field=="category" && $ptype=="register")
	{
		return "Class=50";
	}
				if($table=="my_timetable" && $field=="category" && $ptype=="add")
	{
		return "Class=30";
	}
				if($table=="my_timetable" && $field=="category" && $ptype=="search")
	{
		return "Class=30";
	}
				if($table=="my_timetable" && $field=="level")
	{
		return "Class=40";
	}
				if($table=="my_timetable" && $field=="department")
	{
		return "Class=20";
	}
				if($table=="User Access Level" && $field=="pos")
	{
		return "Class=50";
	}
	return "";
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="timetable" && $field=="sstatus" && $ptype=="edit") 
	{
		return "Pending";
	}
				if($table=="timetable" && $field=="sstatus" && $ptype=="add") 
	{
		return "Pending";
	}
				if($table=="timetable" && $field=="sstatus" && $ptype=="search") 
	{
		return "Pending";
	}
				if($table=="timetable" && $field=="date_status") 
	{
		return Now();
	}
				if($table=="user" && $field=="sstatus") 
	{
		return "Pending";
	}
				if($table=="my_timetable" && $field=="sstatus") 
	{
		return "Pending";
				...
				if($table=="User Access Level" && $field=="sstatus" && $ptype=="add") 
	{
		return "Active";
	}
				if($table=="feedback" && $field=="dates") 
	{
		return Now();
	}
	return "";
...
function runner_mail_smtp($params)
{
	...
	if ("0" == "1" && "kopiah2@gmail.com"!="" || isset($params['username']))
	{
		$mail->SMTPAuth   = true;                  				// enable SMTP authentication
		$mail->Username   = isset($params['username']) ? $params['username'] : "kopiah2@gmail.com";  		// SMTP username
		$mail->Password   = isset($params['password']) ? $params['password'] : "gengmalay86";   // SMTP password
	}else{
		$mail->SMTPAuth = false;
	}
	if("0" == "1" && "smtp.gmail.com" != "" || isset($params['host']))
	{
		$mail->Host = isset($params['host']) ? $params['host'] : "smtp.gmail.com";     // sets SMTP server
	}
	else if(ini_get('SMTP') != '')
		$mail->Host = ini_get('SMTP');
		
	if("0" == "1" && "465" != "" || isset($params['port']))
		$mail->Port = isset($params['port']) ? $params['port'] + 0 : "465"+0;     	// set the SMTP port
