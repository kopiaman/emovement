...
function checkTableName($shortTName, $type=false)
{
	...
	if ("set_setting_category" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("timetable" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("user" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("user_profile" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("alltest" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("my_timetable" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("department" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("department_sub" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("User_Access_Level" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("User_Level" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("training" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("feedback" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	...
	$strPerm = GetUserPermissions("timetable");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="timetable";
	}
	...
	$strPerm = GetUserPermissions("user_profile");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="user_profile";
	}
	$strPerm = GetUserPermissions("alltest");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="alltest";
	}
	$strPerm = GetUserPermissions("my_timetable");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="my_timetable";
	}
	...
	$strPerm = GetUserPermissions("department_sub");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="department_sub";
	}
	$strPerm = GetUserPermissions("User Access Level");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="User Access Level";
	}
	$strPerm = GetUserPermissions("User_Level");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="User_Level";
	}
	...
	$strPerm = GetUserPermissions("feedback");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="feedback";
	}
...
function ReadUserPermissions($userID="")
{
	...
	if($userID!="Guest")
	{
			...
			$sql = "select `GroupID`, `UserName` from `emov_ugmembers` where `UserName`=".db_prepare_string($userID);
		else
			$sql = "select `GroupID`, `UserName` from `emov_ugmembers` where `UserName`=".strtoupper(db_prepare_string($userID));
		$rs = db_query($sql,$conn);
	...
	$rights=array();
	$sql = "select `TableName`,`AccessMask` from `emov_ugrights` where `GroupID` in (".$groupstr.")";
	$rs = db_query($sql,$conn);
	while($data=db_fetch_numarray($rs))
	{
...
{	
	...
	if(array_key_exists("set_setting_category",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("timetable",$_SESSION["UserRights"]["Guest"]))
		return true;
	...
	if(array_key_exists("user",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("user_profile",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("alltest",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("my_timetable",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("department",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("department_sub",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("User Access Level",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("User_Level",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("training",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("feedback",$_SESSION["UserRights"]["Guest"]))
		return true;
	return false;
}
...
{
	global $globalEvents;
		$_SESSION["GroupID"] = $data["icno"];


			$_SESSION["OwnerID"] = $data["id"];
		$_SESSION["_user_profile_OwnerID"] = $data["id"];
