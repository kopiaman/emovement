...
add_nocache_headers();
include("include/department_sub_variables.php");
...
if($inlineadd == ADD_INLINE)
	$templatefile = "department_sub_inline_add.htm";
else
	$templatefile = "department_sub_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: department_sub_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing subname - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_subname = $pageObject->getControl("subname", $id);
		$control_subname->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing subname - end
//	processing subcode - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_subcode = $pageObject->getControl("subcode", $id);
		$control_subcode->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing subcode - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="department")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["dip"]==""){
			$avalues["dip"] = prepare_for_db("dip",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing subname - start
			...
			if($inlineAddOption)
			{
				$control_subname->afterSuccessfulSave();
			}
//	processing subname - end
//	processing subcode - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_subcode->afterSuccessfulSave();
			}
//	processing subcode - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'department_sub_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'department_sub_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: department_sub_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["subid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["subid"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="department")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["dip"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if(postvalue("table")=="admin_users" && postvalue("field")=="subdepartment") 
	$defvalues["dip"] = postvalue("category");
if(postvalue("table")=="user" && postvalue("field")=="subdepartment") 
	$defvalues["dip"] = postvalue("category");
if(postvalue("table")=="user_profile" && postvalue("field")=="subdepartment") 
	$defvalues["dip"] = postvalue("category");
if(postvalue("table")=="User_Access_Level" && postvalue("field")=="subdepartment") 
	$defvalues["dip"] = postvalue("category");
if($readavalues)
{
}
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["subid"]));
...
//	subname
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("subname", $data, $keylink);
		$showValues["subname"] = $value;
		$showFields[] = "subname";
	}	
//	subcode
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("subcode", $data, $keylink);
		$showValues["subcode"] = $value;
		$showFields[] = "subcode";
	}	
		$showRawValues["subid"] = substr($data["subid"],0,100);
		$showRawValues["dip"] = substr($data["dip"],0,100);
		$showRawValues["subname"] = substr($data["subname"],0,100);
		$showRawValues["subcode"] = substr($data["subcode"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "department_sub";
		$options['firstTime'] = 1;
