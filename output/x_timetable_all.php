...
do{
	...
	echo  '<td bgcolor="'.$color_staff.'"  rowspan="1" style="border:1px solid black; border-collapse:collapse">'.$row_staff['depname'].'</td>';
			...
			$col2=1;	
			do{  
				echo '<td  scope="col"  style="border:dashed 1px silver;border-top:none; border-left:none;"></td>'; 
	...
	$sql_content="	
		...
		WHERE MONTH(timetable.date)=$month	 AND YEAR(timetable.date)=$year AND timetable.staffID=$staffID AND timetable.sstatus='Approved' AND DAY(timetable.date)=$col2
		...
		$sday=$row_content['sday'];  $endate=$row_content['eday'];
		$staff=$row_content['staff']; 	$title=$row_content['title'];$organizer=$row_content['organizer'];
		$category=$row_content['category'];$color=$row_content['color'];$location=$row_content['location'];
		$stime=$row_content['stime'];$etime=$row_content['etime'];
						
		$stime=date("H:i", strtotime($stime)); 
		$etime=date("H:i", strtotime($etime)); 
		 								
		 $content=$title.'<br><strong>'.$organizer.'</strong><br>'
		  .'<strong>'.$location.' </strong><br>('.$sday.'hb) '.$stime.' - '.$etime;
