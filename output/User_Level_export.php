...
$layout = new TLayout("export","ExtravaganzaOrange","MobileOrange");
...
$layout->blocks["top"][] = "export";$page_layouts["User_Level_export"] = $layout;
...
$xt->display("User_Level_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=User_Level.xls");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=User_Level.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["GroupID"] = $pageObject->showDBValue("GroupID", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=User_Level.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["GroupID"] = $pageObject->getViewControl("GroupID")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["GroupID"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Group ID").'</td>';	
	}
	else
	{
