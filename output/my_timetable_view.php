...
$layout->blocks["top"][] = "details";$page_layouts["my_timetable_view"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["my_timetable_view_Date_Time1"] = $layout;
...
if (!sizeof($data)) {
	header("Location: my_timetable_list.php?a=return");
	exit();
}
...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "date_status";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("date_status");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "note";
...
//date_status - Datetime
	...
	if($mainTableOwnerID=="date_status")
		$ownerIdValue=$value;
	$xt->assign("date_status_value",$value);
	if(!$pageObject->isAppearOnTabs("date_status"))
		$xt->assign("date_status_fieldblock",true);
	else
		$xt->assign("date_status_tabfieldblock",true);
////////////////////////////////////////////
//note - 
...
if($pageObject->isShowDetailTables && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "my_timetable";
		$options['firstTime'] = 1;
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='my_timetable_edit.php?".$editlink."'\"");
