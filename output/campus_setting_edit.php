...
$templatefile = ($inlineedit == EDIT_INLINE) ? "campus_setting_inline_edit.htm" : "campus_setting_edit.htm";
...
//	processing acronyim - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_acronyim = $pageObject->getControl("acronyim", $id);
		$control_acronyim->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing acronyim - end
//	processing campus_name - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_campus_name = $pageObject->getControl("campus_name", $id);
		$control_campus_name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing campus_name - end
...
//	processing faxNo - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_faxNo = $pageObject->getControl("faxNo", $id);
		$control_faxNo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing faxNo - end
//	processing telephone - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_telephone = $pageObject->getControl("telephone", $id);
		$control_telephone->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing telephone - end
...
//	processing url - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_url = $pageObject->getControl("url", $id);
		$control_url->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing url - end
	...
	//	do event
		...
		{		
			...
			//	processing acronyim - begin
				...
				if($condition)
				{
					$control_acronyim->afterSuccessfulSave();
				}
	//	processing acronyim - end
			//	processing campus_name - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_campus_name->afterSuccessfulSave();
				}
	//	processing campus_name - end
			//	processing address - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
	...
	//	processing state - end
			//	processing faxNo - begin
				...
				if($condition)
				{
					$control_faxNo->afterSuccessfulSave();
				}
	//	processing faxNo - end
			//	processing telephone - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_telephone->afterSuccessfulSave();
				}
	//	processing telephone - end
			//	processing email - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
	...
	//	processing email - end
			//	processing url - begin
				...
				if($condition)
				{
					$control_url->afterSuccessfulSave();
				}
	//	processing url - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: campus_setting_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: campus_setting_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["acronyim"] = $evalues["acronyim"];
	$data["campus_name"] = $evalues["campus_name"];
	$data["address"] = $evalues["address"];
	$data["city"] = $evalues["city"];
	$data["postcode"] = $evalues["postcode"];
	$data["state"] = $evalues["state"];
	$data["faxNo"] = $evalues["faxNo"];
	$data["telephone"] = $evalues["telephone"];
	$data["email"] = $evalues["email"];
	$data["url"] = $evalues["url"];
}
...
//	logo - 
	$value = $pageObject->showDBValue("logo", $data, $keylink);
	$showValues["logo"] = $value;
	$showFields[] = "logo";
		$showRawValues["logo"] = substr($data["logo"],0,100);

//	acronyim - 
	$value = $pageObject->showDBValue("acronyim", $data, $keylink);
	$showValues["acronyim"] = $value;
	$showFields[] = "acronyim";
		$showRawValues["acronyim"] = substr($data["acronyim"],0,100);

//	campus_name - 
	$value = $pageObject->showDBValue("campus_name", $data, $keylink);
	$showValues["campus_name"] = $value;
	$showFields[] = "campus_name";
		$showRawValues["campus_name"] = substr($data["campus_name"],0,100);
...
//	faxNo - 
	...
	$showValues["faxNo"] = $value;
	$showFields[] = "faxNo";
		$showRawValues["faxNo"] = substr($data["faxNo"],0,100);

//	telephone - 
	$value = $pageObject->showDBValue("telephone", $data, $keylink);
	$showValues["telephone"] = $value;
	$showFields[] = "telephone";
		$showRawValues["telephone"] = substr($data["telephone"],0,100);
...
//	url - 
	$value = $pageObject->showDBValue("url", $data, $keylink);
	$showValues["url"] = $value;
	$showFields[] = "url";
		$showRawValues["url"] = substr($data["url"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "campus_setting";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='campus_setting_view.php?".$viewlink."'\"");
