...
if($inlineadd == ADD_INLINE)
	$templatefile = "User_Access_Level_inline_add.htm";
else
	$templatefile = "User_Access_Level_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: User_Access_Level_add.php');
...
//	processing umail - start
	...
	if($inlineAddOption)
	{
		...
		$control_umail->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing subdepartment - start
	...
	if($inlineAddOption)
	{
		...
		$control_subdepartment->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing pos - start
	...
	if($inlineAddOption)
	{
		...
		$control_pos->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing pos - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'User_Access_Level_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'User_Access_Level_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: User_Access_Level_".$pageObject->getPageType().".php");
