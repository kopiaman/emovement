...
$templatefile = ($inlineedit == EDIT_INLINE) ? "setting_inline_edit.htm" : "setting_edit.htm";
...
//	processing Field_Name - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Field_Name = $pageObject->getControl("Field_Name", $id);
		$control_Field_Name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Field_Name - end
//	processing Value - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Value = $pageObject->getControl("Value", $id);
		$control_Value->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: setting_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: setting_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["Field_Name"] = $evalues["Field_Name"];
	$data["Value"] = $evalues["Value"];
}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='setting_view.php?".$viewlink."'\"");
