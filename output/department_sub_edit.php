...
$templatefile = ($inlineedit == EDIT_INLINE) ? "department_sub_inline_edit.htm" : "department_sub_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["subid"] = urldecode(postvalue("editid1"));
$savedKeys["subid"] = urldecode(postvalue("editid1"));
...
//	processing subname - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_subname = $pageObject->getControl("subname", $id);
		$control_subname->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing subname - end
//	processing subcode - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_subcode = $pageObject->getControl("subcode", $id);
		$control_subcode->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["subid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: department_sub_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: department_sub_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["subname"] = $evalues["subname"];
	$data["subcode"] = $evalues["subcode"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("subid", $data)));
...
//	subid - 
	$value = $pageObject->showDBValue("subid", $data, $keylink);
	$showValues["subid"] = $value;
	$showFields[] = "subid";
		$showRawValues["subid"] = substr($data["subid"],0,100);

//	dip - 
	$value = $pageObject->showDBValue("dip", $data, $keylink);
	$showValues["dip"] = $value;
	$showFields[] = "dip";
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='department_sub_view.php?".$viewlink."'\"");
