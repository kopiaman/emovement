...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["setting_detailspreview"] = $layout;
...
if($mastertable == "set_setting_category")
{
	$where = "";
		$where .= GetFullFieldName("Class", $strTableName, false)."=".make_db_value("Class",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	Field_Name - 
		    ...
		    $value = $viewContainer->showDBValue("Field_Name", $data, $keylink);
			$row["Field_Name_value"] = $value;
	//	Value - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Value", $data, $keylink);
			$row["Value_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("setting_detailspreview.htm");
