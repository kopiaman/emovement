...
if($inlineadd == ADD_INLINE)
	$templatefile = "setting_inline_add.htm";
else
	$templatefile = "setting_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: setting_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing Field_Name - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Field_Name = $pageObject->getControl("Field_Name", $id);
		$control_Field_Name->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Field_Name - end
//	processing Value - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Value = $pageObject->getControl("Value", $id);
		$control_Value->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Value - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="set_setting_category")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["Class"]==""){
			$avalues["Class"] = prepare_for_db("Class",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing Field_Name - start
			...
			if($inlineAddOption)
			{
				$control_Field_Name->afterSuccessfulSave();
			}
//	processing Field_Name - end
//	processing Value - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_Value->afterSuccessfulSave();
			}
//	processing Value - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'setting_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'setting_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: setting_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="set_setting_category")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["Class"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
//	Field_Name
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Field_Name", $data, $keylink);
		$showValues["Field_Name"] = $value;
		$showFields[] = "Field_Name";
	}	
//	Value
	$display = false;
	if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY || $inlineadd==ADD_POPUP)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Value", $data, $keylink);
		$showValues["Value"] = $value;
		$showFields[] = "Value";
	}	
		$showRawValues["ID"] = substr($data["ID"],0,100);
		$showRawValues["Field_Name"] = substr($data["Field_Name"],0,100);
		$showRawValues["Class"] = substr($data["Class"],0,100);
		$showRawValues["Value"] = substr($data["Value"],0,100);
