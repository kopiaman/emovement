...
$layout = new TLayout("detailspreview","ExtravaganzaOrange1","MobileOrange1");
...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["department_sub_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("dip", $strTableName, false)."=".make_db_value("dip",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	subname - 
		    ...
		    $value = $viewContainer->showDBValue("subname", $data, $keylink);
			$row["subname_value"] = $value;
	//	subcode - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("subcode", $data, $keylink);
			$row["subcode_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("department_sub_detailspreview.htm");
