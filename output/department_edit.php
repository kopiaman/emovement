...
include("include/department_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "department_inline_edit.htm" : "department_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["dip"] = urldecode(postvalue("editid1"));
$savedKeys["dip"] = urldecode(postvalue("editid1"));
...
//	processing dname - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_dname = $pageObject->getControl("dname", $id);
		$control_dname->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing dname - end
//	processing dcode - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_dcode = $pageObject->getControl("dcode", $id);
		$control_dcode->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing dcode - end
	...
	//	do event
		...
		{		
			...
			//	processing dname - begin
				...
				if($condition)
				{
					$control_dname->afterSuccessfulSave();
				}
	//	processing dname - end
			//	processing dcode - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_dcode->afterSuccessfulSave();
				}
	//	processing dcode - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["dip"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: department_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: department_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["dname"] = $evalues["dname"];
	$data["dcode"] = $evalues["dcode"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("dip", $data)));
...
//	detail tables
	$showDetailKeys["department_sub"]["masterkey1"] = $data["dip"];		

	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["dip"]));
...
//	dname - 
	$value = $pageObject->showDBValue("dname", $data, $keylink);
	$showValues["dname"] = $value;
	$showFields[] = "dname";
		$showRawValues["dname"] = substr($data["dname"],0,100);

//	dcode - 
	$value = $pageObject->showDBValue("dcode", $data, $keylink);
	$showValues["dcode"] = $value;
	$showFields[] = "dcode";
		$showRawValues["dcode"] = substr($data["dcode"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "department";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='department_view.php?".$viewlink."'\"");
