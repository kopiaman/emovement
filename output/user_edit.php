...
$layout = new TLayout("edit2","ExtravaganzaOrange","MobileOrange");
...
$layout->blocks["top"][] = "details";$page_layouts["user_edit"] = $layout;
...
//	processing id - end
//	processing pass - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_pass = $pageObject->getControl("pass", $id);
		$control_pass->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing pos - end
	...
	//	processing id - end
			//	processing pass - begin
				...
				if($condition)
				{
					$control_pass->afterSuccessfulSave();
				}
	//	processing pass - end
			//	processing fullname - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: user_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: user_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("pass"))
		$xt->assign("pass_fieldblock",true);
	else
		$xt->assign("pass_tabfieldblock",true);
	$xt->assign("pass_label",true);
	if(isEnableSection508())
		$xt->assign_section("pass_label","<label for=\"".GetInputElementId("pass", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("umail"))
		$xt->assign("umail_fieldblock",true);
	else
		$xt->assign("umail_tabfieldblock",true);
	$xt->assign("umail_label",true);
	if(isEnableSection508())
		$xt->assign_section("umail_label","<label for=\"".GetInputElementId("umail", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("pos"))
		$xt->assign("pos_fieldblock",true);
	else
		$xt->assign("pos_tabfieldblock",true);
	$xt->assign("pos_label",true);
	if(isEnableSection508())
		$xt->assign_section("pos_label","<label for=\"".GetInputElementId("pos", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='user_view.php?".$viewlink."'\"");
